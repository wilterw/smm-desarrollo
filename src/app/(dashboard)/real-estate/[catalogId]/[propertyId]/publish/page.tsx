...
export default function PublishWizard({ params }: { params: Promise<{ catalogId: string, propertyId: string }> }) {
  ...
  const [publishType, setType] = useState<"organic" | "ads" | "catalog_sync" | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/real-estate/properties?id=${resolvedParams.propertyId}`).then(r => r.json()).then(foundAd => {
  ...
  const handleNext = () => {
    ...
    if (step === 3 && publishType === 'catalog_sync') {
      setStep(5); // Directly to publish step
      return;
    }
  ...
  const handlePublish = async () => {
    ...
    setError(null);
    try {
      // Auto-save property state (Description, Link) before publishing
      let finalDesc = ad.description;
      await fetch(`/api/real-estate/properties`, {
        ...
        body: JSON.stringify({
          ...
          listingUrl: ad.listingUrl
        })
      });

      const res = await fetch("/api/real-estate/publish", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          propertyId: ad.id,
          destinations: publishType === 'catalog_sync' ? [{ platform: 'meta', publishType: 'catalog_sync', destination: 'catalog' }] : selectedAccountIds.map(accountId => {
    ...
    return (
      <div className={styles.destinationGrid}>
        {[
          ...
          { id: 'catalog_sync', name: 'Sync Catálogo', icon: '🔄', desc: 'XML Advantage+' }
        ].map(t => (
       ...
       <div className={styles.previewContainer}>
          ...
          <div className={styles.fbPost}>
             ...
             <div className={styles.fbMediaGrid} style={(origin === 'reels' || origin === 'stories') ? { aspectRatio: "9/16", maxHeight: "600px" } : {}}>
                {ad.imageUrl && (
                   <img src={ad.imageUrl} alt="Preview" className={styles.fbMediaItem} style={(origin === 'reels' || origin === 'stories') ? { objectFit: "cover", height: "100%" } : {}} />
                )}
             </div>
    ...
    <div className={styles.adsTerminal}>
      <div className={styles.configColumn}>
        ...
        <div className={styles.metaCard}>
           ...
           <p style={{ fontSize: "0.85rem", opacity: 0.7, marginBottom: "0.5rem" }}>El contenido gráfico principal de tu anuncio.</p>
           {ad.imageUrl ? (
             <div style={{ borderRadius: "var(--radius-md)", overflow: "hidden", border: "1px solid var(--border-color)", width: "100%", height: "200px", background: "var(--bg-primary)", display: "flex", alignItems: "center", justifyContent: "center" }}>
               <img src={ad.imageUrl} alt="Property Image" style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain" }} />
             </div>
           ) : (
        ...
        <div className={styles.metaCard}>
           ...
           <div style={{ marginTop: "1rem" }}>
              <p style={{ fontSize: "0.85rem", fontWeight: 700, marginBottom: "0.25rem" }}>Ciudad</p>
              <input 
                type="text"
                className={styles.input} 
                value={ad.city || ""}
         ...
         <div className={styles.metaCard}>
            ...
            <select 
              ...
              onChange={(e) => {
                ...
                if (newCta === 'MESSAGE_PAGE' || newCta === 'WHATSAPP_MESSAGE') {
            ...
            >
               <option value="MESSAGE_PAGE">Enviar mensaje</option>
        ...
        <div className={styles.metaCard}>
           ...
           <input 
             ...
             value={ad.listingUrl || ""}
             onChange={(e) => setAd({...ad, listingUrl: e.target.value})}
           />
        </div>
      ...
      <div className={styles.mainLayout} style={{ gridTemplateColumns: (step === 5 && publishType === 'ads' && platform === 'facebook') ? '64px 1fr' : undefined }}>
        ...
        <div className={styles.centerArea} style={{ 
          ...
          <div className={styles.cardHeader} style={{ background: "transparent", border: "none" }}>
             ...
             <Link href={`/real-estate/${resolvedParams.catalogId}`} className={styles.btnSecondary} style={{ padding: "0.4rem 1rem", fontSize: "0.8rem" }}>← Volver</Link>
          </div>
          ...
          {publishResults ? (
            <div className={styles.configCard}>
              ...
              <button className={styles.btnPrimary} style={{ marginTop: "1rem" }} onClick={() => router.push(`/real-estate/${resolvedParams.catalogId}`)}>Cerrar</button>
            </div>
          ) : (
            <>
              {step === 1 && (
